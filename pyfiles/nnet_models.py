...
class BagOfWords(nn.Module):
    ...
    def forward(self, x, xs_len=None, lang=None):
        ...
        return outputs[0], torch.cat(outputs, dim=0)
...
class AttentionDecoder(nn.Module):
    ...
    def forward(
        ...
        return return_scores.transpose(
            0, 1).contiguous(), memory.transpose(
            0, 1), torch.stack(attn_wts_list, axis=1), context_vec
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, input, hidden=None, xs_len=None, lang=None):
        ...
        return outputs[-1], torch.cat(outputs, dim=0)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        return scores, None, attn_weights, context_vec
...
class EncoderDecoder(nn.Module):
    ...
    def train_step(self, batch):
        ...
        encoder_output, encoder_hidden = self.encoder(xs, xs_len=xs_len, lang=source_lang)

        # Teacher forcing: Feed the target as the next input
        y_in = ys.narrow(1, 0, ys.size(1) - 1)
        decoder_input = torch.cat([starts, y_in], 1)

        decoder_output, _, _, _ = self.decoder(
            decoder_input,
            encoder_hidden,
            encoder_output,
            xs_len,
    ...
    def eval_step(self, batch):
        ...
        for i in range(self.max_len):
            ...
            decoder_output, decoder_hidden, attn_wts, context_vec = self.decoder(
...
class TransformerDecoderLayer(nn.TransformerDecoderLayer):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None,
        ...
        return x, self_attn_weights, attn_weights
